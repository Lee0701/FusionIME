...
class SwipeTrajectoryProcessor {
    ...
    private fun normalizeCoordinates(coordinates: List<PointF>, outNormalized: ArrayList<PointF>) {
        ...
        if (debugModeActive && coordinates.isNotEmpty() && outNormalized.isNotEmpty()) {
            ...
            sb.append("─────────────────────────────────────────────────────────\n")
            sb.append("   Raw X range: [${String.format(Locale.ROOT, "%.1f", minRawX)}, ${String.format(
                Locale.ROOT, "%.1f", maxRawX)}] px\n")
            sb.append("   Raw Y range: [${String.format(Locale.ROOT, "%.1f", minRawY)}, ${String.format(
                Locale.ROOT, "%.1f", maxRawY)}] px\n")
            ...
            for (i in sampleIndices) {
                ...
                sb.append("     $label: raw=(${String.format(Locale.ROOT, "%.1f", raw.x)}, ${String.format(
                    Locale.ROOT, "%.1f", raw.y)}) → norm=(${String.format(Locale.ROOT, "%.3f", norm.x)}, ${String.format(
                    Locale.ROOT, "%.3f", norm.y)})\n")
            }
