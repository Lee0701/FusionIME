...
object Researcher {
        ...
        private suspend fun processWithTones(keys: List<VirtualInputKey>, lexicons: List<Lexicon>): List<Lexicon> {
        ...
        private suspend fun processWithSeparators(keys: List<VirtualInputKey>, lexicons: List<Lexicon>): List<Lexicon> {
        ...
        private suspend fun search(keys: List<VirtualInputKey>, text: String, segmentation: Segmentation, limit: Int? = null): List<Lexicon> {
                val inputLength = keys.size
        ...
        private suspend fun performQuery(scheme: Scheme, limit: Int? = null): List<Lexicon> {
                yield()
                val anchors = scheme.originAnchors
                if (anchors.isEmpty()) return emptyList()
                val spellCode = scheme.originText.hashCode()
                return strictMatch(anchors = anchors, spell = spellCode, input = scheme.aliasText, mark = scheme.previewMark, limit = limit)
        }
        ...
        private suspend fun anchorsMatch(keys: List<VirtualInputKey>, input: String? = null, limit: Int? = null): List<Lexicon> {
                ...
                BinaryDictionaries.anchorDict.get(keys.map { it.character }.joinToString("")).forEach { number ->
                        yield()
                        val entry = BinaryDictionaries.vocabDict.get(number)
                        val instance = Lexicon(text = entry.word, romanization = entry.romanization, input = inputText, mark = inputText, number = number)
                        instances.add(instance)
                }
                return instances
        }
        private suspend fun spellMatch(text: String, input: String, mark: String? = null, limit: Int? = null): List<Lexicon> {
                if (text.isBlank()) return emptyList()
                val spellCode: Int = text.hashCode()
                val instances: MutableList<Lexicon> = mutableListOf()
                val limitValue: Int = limit ?: -1
                BinaryDictionaries.spellDict.search(spellCode).forEach { number ->
                        yield()
                        val entry = BinaryDictionaries.vocabDict.get(number)
                        val markText = mark ?: entry.romanization.filterNot { it.isCantoneseToneDigit }
                        val instance = Lexicon(text = entry.word, romanization = entry.romanization, input = input, mark = markText, number = number)
                        instances.add(instance)
                }
                return instances
        }
        private suspend fun strictMatch(anchors: List<VirtualInputKey>, spell: Int, input: String, mark: String? = null, limit: Int? = null): List<Lexicon> {
                if (anchors.isEmpty()) return emptyList()
                val instances: MutableList<Lexicon> = mutableListOf()
                val limitValue: Int = limit ?: -1
                val spellMatch = BinaryDictionaries.spellDict.search(spell).toSet()
                val anchorMatch = BinaryDictionaries.anchorDict.get(anchors.map { it.character }.joinToString("")).toSet()
                spellMatch.filter { it in anchorMatch }.forEach { number ->
                        yield()
                        val entry = BinaryDictionaries.vocabDict.get(number)
                        val markText = mark ?: entry.romanization.filterNot { it.isCantoneseToneDigit }
                        val instance = Lexicon(text = entry.word, romanization = entry.romanization, input = input, mark = markText, number = number)
