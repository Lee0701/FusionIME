...
object MobileKeyboard {
    ...
    fun alphabetic(
        ...
        val rows = mutableListOf<List<KeyboardConfiguration.Item>>()
        if(numberRow) rows += listOf(KeyboardConfiguration.Item.ContentRow(3))
        rows += listOf(KeyboardConfiguration.Item.ContentRow(2))
