 ...
 */
    ...
    companion object {
        ...
        fun convert(text: ComposingText, table: HashMap<String, String>): Boolean {
            ...
            val str = arrayOfNulls<StrSegment>(MAX_LENGTH)
            var start = MAX_LENGTH
            val checkLength = min(
                cursor.toDouble(),
                MAX_LENGTH.toDouble()
            ).toInt()
            ...
            while (start < MAX_LENGTH) {
                for (i in start until MAX_LENGTH) {
                    key.append(str[i]!!.string)
                }
                val upper = Character.isUpperCase(key[key.length - 1])
                var match = table[key.toString().lowercase(Locale.getDefault())]
                if (match != null) {
                    if (upper) {
                        match = match.uppercase(Locale.getDefault())
                    }
                    val out: Array<StrSegment?>
                    if (match.length == 1) {
                        out = arrayOfNulls(1)
