...
class KanaConverter {
    ...
    private val mAddCandidateList: MutableList<WnnWord> =
        ArrayList()

    /** Work area for generating string  */
    private val mStringBuff = StringBuffer()

    /** part of speech (default)  */
    private var mPosDefault: WnnPOS? = null

    /** part of speech (number)  */
    private var mPosNumber: WnnPOS? = null

    /** part of speech (symbol)  */
    private var mPosSymbol: WnnPOS? = null

    /**
     * Set The dictionary.
     * <br></br>
     * [KanaConverter] gets part-of-speech tags from the dictionary.
     *
     ...
     */
    fun createPseudoCandidateList(
        inputHiragana: String,
        inputRomaji: String,
        keyBoardMode: Int
    ): List<WnnWord> {
        val list = mAddCandidateList
        ...
        list.add(WnnWord(inputHiragana, inputHiragana))
        ...
        if (keyBoardMode == OpenWnnEngineJAJP.KEYBOARD_QWERTY) {
            ...
            if (createCandidateString(inputHiragana, mHalfNumericMap, mStringBuff)) {
                val convHanSuuji = mStringBuff.toString()
                val convNumComma = convertNumber(convHanSuuji)
                list.add(WnnWord(convHanSuuji, inputHiragana, mPosNumber))
                if (convNumComma != null) {
                    list.add(WnnWord(convNumComma, inputHiragana, mPosNumber))
                }
            }

            /* Create pseudo candidates for full width numeric */
            if (createCandidateString(inputHiragana, mFullNumericMap, mStringBuff)) {
                list.add(WnnWord(mStringBuff.toString(), inputHiragana, mPosNumber))
            }

            /* Create pseudo candidates for half width alphabet */
            if (createCandidateString(inputHiragana, mHalfAlphabetMap, mStringBuff)) {
                val convHanEiji = mStringBuff.toString()
                val convHanEijiLower = convHanEiji.lowercase(Locale.getDefault())
                list.add(WnnWord(convHanEijiLower, inputHiragana, mPosSymbol))
                list.add(WnnWord(convertCaps(convHanEijiLower), inputHiragana, mPosSymbol))
                list.add(WnnWord(convHanEiji, inputHiragana, mPosSymbol))
            }

            /* Create pseudo candidates for full width alphabet */
            if (createCandidateString(inputHiragana, mFullAlphabetMap, mStringBuff)) {
                val convZenEiji = mStringBuff.toString()
                val convZenEijiLower = convZenEiji.lowercase(Locale.JAPAN)
                list.add(WnnWord(convZenEijiLower, inputHiragana, mPosSymbol))
                list.add(WnnWord(convertCaps(convZenEijiLower), inputHiragana, mPosSymbol))
                list.add(WnnWord(convZenEiji, inputHiragana, mPosSymbol))
            }
        }
        return list
    }
     ...
     * @param inputRomaji       The input string (Romaji)
     */
    private fun createPseudoCandidateListForQwerty(inputHiragana: String, inputRomaji: String) {
        val list = mAddCandidateList

        /* Create pseudo candidates for half width alphabet */
        val convHanEijiLower = inputRomaji.lowercase(Locale.getDefault())
        list.add(WnnWord(inputRomaji, inputHiragana, mPosDefault))
        list.add(WnnWord(convHanEijiLower, inputHiragana, mPosSymbol))
        list.add(WnnWord(convertCaps(convHanEijiLower), inputHiragana, mPosSymbol))
        list.add(WnnWord(inputRomaji.uppercase(Locale.getDefault()), inputHiragana, mPosSymbol))
        ...
        if (createCandidateString(inputRomaji, mFullAlphabetMapQwety, mStringBuff)) {
            ...
            list.add(WnnWord(convZenEijiLower, inputHiragana, mPosSymbol))
            list.add(WnnWord(convertCaps(convZenEijiLower), inputHiragana, mPosSymbol))
            list.add(WnnWord(convZenEiji.uppercase(Locale.JAPAN), inputHiragana, mPosSymbol))
        }
    }
     ...
     */
    private fun createCandidateString(
        input: String,
        ...
        for (index in 0 until input.length) {
            val convChar = map[input.substring(index, index + 1)] ?: return false
     ...
     */
    private fun convertCaps(moji: String?): String {
        var tmp = ""
        if (moji != null && moji.length > 0) {
            tmp = (moji.substring(0, 1).uppercase(Locale.JAPAN)
                    + moji.substring(1).lowercase(Locale.JAPAN))
        }
        return tmp
    }
     ...
     * @return          `true` if success
     */
    private fun convertNumber(numComma: String): String? {
        return try {
            mFormat.format(numComma.toDouble())
