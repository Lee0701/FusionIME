...
class SetupWizardActivity : Activity(), View.OnClickListener {
    ...
    private var mWelcomeVideoUri: Uri? = null
    private var mWelcomeVideoView: VideoView? = null
    private var mWelcomeImageView: ImageView? = null
    private var mActionStart: View? = null
    private var mActionNext: View? = null
    private var mStep1Bullet: TextView? = null
    private var mActionFinish: TextView? = null
    private var mSetupStepGroup: SetupStepGroup? = null
    private var mStepNumber = 0
    private var mNeedsToAdjustStepNumberToSystemState = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val indicatorView =
            findViewById<View>(R.id.setup_step_indicator) as SetupStepIndicatorView
        ...
        mWelcomeVideoUri = Uri.Builder()
            .scheme(ContentResolver.SCHEME_ANDROID_RESOURCE)
            .authority(packageName)
            .path(R.raw.setup_welcome_video.toString())
            .build()
        val welcomeVideoView = findViewById<View>(R.id.setup_welcome_video) as VideoView
        welcomeVideoView.setOnPreparedListener { mp -> // Now VideoView has been laid-out and ready to play, remove background of it to
            // reveal the video.
        ...
        welcomeVideoView.setOnErrorListener { mp, what, extra ->
            Log.e(
                TAG,
                "Playing welcome video causes error: what=$what extra=$extra"
            )
    ...
    override fun onClick(v: View) {
        ...
        val nextStep = if (v === mActionStart) {
            STEP_1
    ...
    fun hideWelcomeVideoAndShowWelcomeImage() {
        ...
        mWelcomeImageView!!.visibility = View.VISIBLE
    }

    private fun showAndStartWelcomeVideo() {
        mWelcomeVideoView!!.visibility = View.VISIBLE
        mWelcomeVideoView!!.setVideoURI(mWelcomeVideoUri)
        mWelcomeVideoView!!.start()
    }

    private fun hideAndStopWelcomeVideo() {
    ...
    private fun updateSetupStepView() {
        ...
        mWelcomeScreen!!.visibility =
            if (welcomeScreen) View.VISIBLE else View.GONE
        mSetupScreen!!.visibility =
            if (welcomeScreen) View.GONE else View.VISIBLE
        if (welcomeScreen) {
        ...
        mActionNext!!.visibility =
            if (isStepActionAlreadyDone) View.VISIBLE else View.GONE
        mActionFinish!!.visibility =
            if ((mStepNumber == STEP_3)) View.VISIBLE else View.GONE
    }

    internal class SetupStep(
        stepNo: Int, applicationName: String?, bulletView: TextView?,
        stepView: View, title: Int, instruction: Int,
        finishedInstruction: Int, actionIcon: Int, actionLabel: Int
    ) :
        View.OnClickListener {
        val mStepNo: Int = stepNo
        private val mStepView = stepView
        private val mBulletView = bulletView
        ...
        fun setEnabled(enabled: Boolean, isStepActionAlreadyDone: Boolean) {
            ...
            instructionView.text =
                if (isStepActionAlreadyDone) mFinishedInstruction else mInstruction
            mActionLabel.visibility =
                if (isStepActionAlreadyDone) View.GONE else View.VISIBLE
        }
    ...
    internal class SetupStepGroup(indicatorView: SetupStepIndicatorView) {
    ...
    companion object {
        val TAG: String = SetupWizardActivity::class.java.simpleName

        // For debugging purpose.
        private const val FORCE_TO_SHOW_WELCOME_SCREEN = false
        private const val ENABLE_WELCOME_VIDEO = true
