 ...
 */
class DictionaryPackInstallBroadcastReceiver : BroadcastReceiver {
    val mService: LatinIME?
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (action == Intent.ACTION_PACKAGE_ADDED) {
            if (null == mService) {
                Log.e(
                    TAG, ("Called with intent " + action + " but we don't know the service: this "
                            + "should never happen")
                )
                return
            }
            val packageUri: Uri? = intent.getData()
            ...
            val providers: Array<ProviderInfo>? = packageInfo.providers
            ...
            for (info: ProviderInfo in providers) {
                if (DictionaryPackConstants.AUTHORITY == info.authority) {
            ...
            mService.resetSuggestMainDict()
        } else if (action == DictionaryPackConstants.NEW_DICTIONARY_INTENT_ACTION) {
            if (null == mService) {
                Log.e(
                    TAG, ("Called with intent " + action + " but we don't know the service: this "
                            + "should never happen")
            ...
            mService.resetSuggestMainDict()
        } else if (action == DictionaryPackConstants.UNKNOWN_DICTIONARY_PROVIDER_CLIENT) {
            if (null != mService) {
                ...
                Log.e(
                    TAG, ("Called with intent " + action + " but we have a reference to the "
            ...
            val wantedClientId: String? =
                intent.getStringExtra(DictionaryPackConstants.DICTIONARY_PROVIDER_CLIENT_EXTRA)
            val myClientId: String = context.getString(R.string.dictionary_pack_client_id)
            if (wantedClientId != myClientId) return  // Not for us
    ...
    companion object {
        private val TAG: String = DictionaryPackInstallBroadcastReceiver::class.java.getSimpleName()
    }
}
