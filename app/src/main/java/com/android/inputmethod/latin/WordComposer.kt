...
class WordComposer {
     ...
     */
    var autoCorrectionOrNull: SuggestedWordInfo?
        private set
    ...
    fun moveCursorByAndReturnIfInsideComposingWord(expectedMoveAmount: Int): Boolean {
        ...
        if (expectedMoveAmount >= 0) {
            ...
            while (actualMoveAmount < expectedMoveAmount && cursorPos < codePoints.size) {
                actualMoveAmount += Character.charCount(codePoints.get(cursorPos))
                ++cursorPos
            }
        } else {
    ...
    fun setBatchInputWord(word: String) {
        ...
        var i: Int = 0
        while (i < length) {
            val codePoint: Int = Character.codePointAt(word, i)
    ...
    companion object {
        private val MAX_WORD_LENGTH: Int = DecoderSpecificConstants.DICTIONARY_MAX_WORD_LENGTH
        private val DBG: Boolean = DebugFlags.DEBUG_ENABLED

        const val CAPS_MODE_OFF: Int = 0

        // 1 is shift bit, 2 is caps bit, 4 is auto bit but this is just a convention as these bits
        // aren't used anywhere in the code
        const val CAPS_MODE_MANUAL_SHIFTED: Int = 0x1
        const val CAPS_MODE_MANUAL_SHIFT_LOCKED: Int = 0x3
        const val CAPS_MODE_AUTO_SHIFTED: Int = 0x5
        const val CAPS_MODE_AUTO_SHIFT_LOCKED: Int = 0x7
    }
}
