...
class InputLogic(
    ...
    private val mSuggestionStripViewAccessor =
    ...
    /* package */
    val mWordComposer: WordComposer = WordComposer()
     ...
     */
    fun onTextInput(
        settingsValues: SettingsValues, event: Event,
        keyboardShiftMode: Int, handler: UIHandler
    ): InputTransaction {
    ...
    fun onUpdateSelection(
        ...
        newSelStart: Int, newSelEnd: Int, settingsValues: SettingsValues
    ): Boolean {
        ...
        val selectionChangedOrSafeToReset =
    ...
    fun onCodeInput(
        ...
        if (!inputTransaction.didAutoCorrect() && processedEvent.mKeyCode != Constants.CODE_SHIFT && processedEvent.mKeyCode != Constants.CODE_CAPSLOCK && processedEvent.mKeyCode != Constants.CODE_SWITCH_ALPHA_SYMBOL) mLastComposedWord.deactivate()
    ...
    fun onStartBatchInput(
        settingsValues: SettingsValues,
        keyboardSwitcher: KeyboardSwitcher, handler: UIHandler
    ) {
        mWordBeingCorrectedByCursor = null
        ...
        if (mWordComposer.isComposingWord) {
            if (mWordComposer.isCursorFrontOrMiddleOfComposingWord) {
                ...
                unlearnWord(
                    mWordComposer.typedWord, settingsValues,
    ...
    private fun handleConsumedEvent(event: Event, inputTransaction: InputTransaction) {
        ...
        if (mWordComposer.isComposingWord) {
            setComposingTextInternal(mWordComposer.typedWord, 1)
            inputTransaction.setDidAffectContents()
    ...
    private fun handleFunctionalEvent(
        event: Event, inputTransaction: InputTransaction,
        currentKeyboardScriptId: Int, handler: UIHandler
    ) {
        when (event.mKeyCode) {
        ...
        if (inputTransaction.mSettingsValues.isWordSeparator(codePoint)
            ...
            if (SpaceState.PHANTOM == inputTransaction.mSpaceState) {
                if (mWordComposer.isCursorFrontOrMiddleOfComposingWord) {
                    ...
                    // We also need to unlearn the original word that is now being corrected.
                    unlearnWord(
                        mWordComposer.typedWord, inputTransaction.mSettingsValues,
                        Constants.EVENT_BACKSPACE
                    )
                    resetEntireInputState(
                        mConnection.expectedSelectionStart,
    ...
    private fun handleNonSeparatorEvent(
        event: Event, settingsValues: SettingsValues,
        inputTransaction: InputTransaction
    ) {
        val codePoint = event.mCodePoint
        ...
        if (!isComposingWord // We only start composing if this is a word code point. Essentially that means it's a
            // a letter or a word connector.
            && settingsValues.isWordCodePoint(codePoint) // We never go into composing state if suggestions are not requested.
            && settingsValues.needsToLookupSuggestions() &&  // In languages with spaces, we only start composing a word when we are not already
    ...
    private fun handleSeparatorEvent(
        ...
        val isInsideDoubleQuoteOrAfterDigit = Constants.CODE_DOUBLE_QUOTE == codePoint
                && mConnection.isInsideDoubleQuoteOrAfterDigit
        val needsPrecedingSpace = if (SpaceState.PHANTOM != inputTransaction.mSpaceState) {
            false
    ...
    private fun handleBackspaceEvent(
        ...
        if (mWordComposer.isComposingWord) {
            if (mWordComposer.isBatchMode) {
                val rejectedSuggestion = mWordComposer.typedWord
                mWordComposer.reset()
                mWordComposer.rejectedBatchModeSuggestion = rejectedSuggestion
            ...
            if (mConnection.hasSelection()) {
                ...
                if (!TextUtils.isEmpty(selection)) {
                    unlearnWord(
                        selection.toString(), inputTransaction.mSettingsValues,
                        Constants.EVENT_BACKSPACE
                    )
                    hasUnlearnedWordBeingDeleted = true
                }
                val numCharsDeleted = (mConnection.expectedSelectionEnd
                        - mConnection.expectedSelectionStart)
                ...
                if (inputTransaction.mSettingsValues.isBeforeJellyBean
                    ...
                    if (mDeleteCount > Constants.DELETE_ACCELERATE_AT) {
                        ...
                        hasUnlearnedWordBeingDeleted =
                            hasUnlearnedWordBeingDeleted or unlearnWordBeingDeleted(
                    ...
                    if (mDeleteCount > Constants.DELETE_ACCELERATE_AT) {
                        ...
                        val codePointBeforeCursorToDeleteAgain =
                            mConnection.codePointBeforeCursor
    ...
    fun getWordAtCursor(settingsValues: SettingsValues, currentKeyboardScriptId: Int): String {
        if (!mConnection.hasSelection()
    ...
    fun unlearnWordBeingDeleted(
        settingsValues: SettingsValues, currentKeyboardScriptId: Int
    ): Boolean {
        if (mConnection.hasSlowInputConnection()) {
    ...
    fun unlearnWord(word: String, settingsValues: SettingsValues, eventType: Int) {
    ...
    private fun tryPerformDoubleSpacePeriod(
        ...
        if (!inputTransaction.mSettingsValues.mUseDoubleSpacePeriod
            || Constants.CODE_SPACE != event.mCodePoint || !isDoubleSpacePeriodCountdownActive(
                inputTransaction
            )
        ) {
            return false
        ...
        val firstCodePoint =
            if (Character.isSurrogatePair(lastTwo[0], lastTwo[1])) Character.codePointAt(
                lastTwo,
                length - 3
            ) else lastTwo[length - 2].code
    ...
    fun performUpdateSuggestionStripSync(
        settingsValues: SettingsValues,
        inputStyle: Int
    ) {
        ...
        if (!settingsValues.needsToLookupSuggestions()) {
            if (mWordComposer.isComposingWord) {
                Log.w(
                    TAG, "Called updateSuggestionsOrPredictions but suggestions were not "
                            + "requested!"
                )
            }
        ...
        val suggestedWords = holder[null, Constants.GET_SUGGESTED_WORDS_TIMEOUT.toLong()]
        ...
        if (DebugFlags.DEBUG_ENABLED) {
            ...
            Log.d(
                TAG,
                "performUpdateSuggestionStripSync() : $runTimeMillis ms to finish"
            )
        }
    }
     ...
     */
    fun restartSuggestionsOnWordTouchedByCursor(
        settingsValues: SettingsValues,
        forStartInput: Boolean,  // TODO: remove this argument, put it into settingsValues
        currentKeyboardScriptId: Int
    ) {
        // HACK: We may want to special-case some apps that exhibit bad behavior in case of
        // recorrection. This is a temporary, stopgap measure that will be removed later.
        // TODO: remove this.
        if (settingsValues.isBrokenByRecorrection // Recorrection is not supported in languages without spaces because we don't know
            ...
            || !settingsValues.needsToLookupSuggestions() // If we are currently in a batch input, we must not resume suggestions, or the result
    ...
    fun doShowSuggestionsAndClearAutoCorrectionIndicator(suggestedWords: SuggestedWords?) {
        mIsAutoCorrectionIndicatorOn = false
    ...
    private fun revertCommit(
        ...
        if (!TextUtils.isEmpty(committedWord)) {
            unlearnWord(
                committedWordString, inputTransaction.mSettingsValues,
                Constants.EVENT_REVERT
            )
        }
        val stringToCommit = originallyTypedWord.toString() +
                (if (usePhantomSpace) "" else separatorString)
        val textToCommit = SpannableString(stringToCommit)
        if (committedWord is SpannableString) {
            val committedWordWithSuggestionSpans = committedWord
            val spans = committedWordWithSuggestionSpans.getSpans(
                0,
            ...
            for (span in spans) {
                ...
                if (span is SuggestionSpan) {
                    ...
                    textToCommit.setSpan(
                        span, 0,  /* start */lastCharIndex,  /* end */
            ...
            textToCommit.setSpan(
                SuggestionSpan(
                    ...
                    suggestions.toTypedArray<String>(), 0,  /* flags */
    ...
    private fun commitCurrentAutoCorrection(
        settingsValues: SettingsValues,
        separator: String, handler: UIHandler
    ) {
    ...
    private fun commitChosenWord(
        ...
        if (DebugFlags.DEBUG_ENABLED) {
            ...
            Log.d(
                TAG,
                "commitChosenWord() : NgramContext = $ngramContext"
            )
            startTimeMillis = System.currentTimeMillis()
        }
    ...
    fun retryResetCachesAndReturnSuccess(
        ...
        val shouldFinishComposition = mConnection.hasSelection()
    ...
    private fun setComposingTextInternalWithBackgroundColor(
        ...
        if (backgroundColor == Color.TRANSPARENT) {
            ...
            val spanLength =
                min(coloredTextLength.toDouble(), spannable.length.toDouble()).toInt()
            spannable.setSpan(
                backgroundColorSpan, 0, spanLength,
    ...
    val composingLength: Int
         ...
         */
        get() = mWordComposer.size()

    companion object {
        private val TAG: String = InputLogic::class.java.simpleName
        ...
        private fun canBeFollowedByDoubleSpacePeriod(codePoint: Int): Boolean {
            ...
            return Character.isLetterOrDigit(codePoint)
                    || codePoint == Constants.CODE_SINGLE_QUOTE || codePoint == Constants.CODE_DOUBLE_QUOTE || codePoint == Constants.CODE_CLOSING_PARENTHESIS || codePoint == Constants.CODE_CLOSING_SQUARE_BRACKET || codePoint == Constants.CODE_CLOSING_CURLY_BRACKET || codePoint == Constants.CODE_CLOSING_ANGLE_BRACKET || codePoint == Constants.CODE_PLUS || codePoint == Constants.CODE_PERCENT || Character.getType(
                codePoint
        ...
        private fun isResumableWord(settings: SettingsValues, word: String): Boolean {
            ...
            return settings.isWordCodePoint(firstCodePoint)
                    && Constants.CODE_SINGLE_QUOTE != firstCodePoint && Constants.CODE_DASH != firstCodePoint
        }

        /**
         * Make a [SuggestedWords] object containing a typed word
         * and obsolete suggestions.
         * See [SuggestedWords.getTypedWordAndPreviousSuggestions].
