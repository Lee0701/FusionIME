 ...
 * strokes.
 */
abstract class Dictionary(dictType: String, locale: Locale?) {
    ...
    val isUserSpecific: Boolean
         ...
         */
        get() {
            return sUserSpecificDictionaryTypes.contains(
                mDictType
            )
        }
     ...
     */
    @UsedForTesting
    class PhonyDictionary @UsedForTesting constructor(type: String) :
    ...
    companion object {
        val NOT_A_PROBABILITY: Int = -1
        val NOT_A_WEIGHT_OF_LANG_MODEL_VS_SPATIAL_MODEL: Float = -1.0f

        // The following types do not actually come from real dictionary instances, so we create
        // corresponding instances.
        const val TYPE_USER_TYPED: String = "user_typed"
        val DICTIONARY_USER_TYPED: PhonyDictionary = PhonyDictionary(TYPE_USER_TYPED)

        const val TYPE_USER_SHORTCUT: String = "user_shortcut"
        val DICTIONARY_USER_SHORTCUT: PhonyDictionary = PhonyDictionary(TYPE_USER_SHORTCUT)

        const val TYPE_APPLICATION_DEFINED: String = "application_defined"
        val DICTIONARY_APPLICATION_DEFINED: PhonyDictionary = PhonyDictionary(
            TYPE_APPLICATION_DEFINED
        )

        const val TYPE_HARDCODED: String = "hardcoded" // punctuation signs and such
        val DICTIONARY_HARDCODED: PhonyDictionary = PhonyDictionary(TYPE_HARDCODED)

        // Spawned by resuming suggestions. Comes from a span that was in the TextView.
        const val TYPE_RESUMED: String = "resumed"
