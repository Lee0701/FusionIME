...
object CapsModeUtils {
    ...
    private fun isStartPunctuation(codePoint: Int): Boolean {
        return (codePoint == Constants.CODE_DOUBLE_QUOTE || codePoint == Constants.CODE_SINGLE_QUOTE || codePoint == Constants.CODE_INVERTED_QUESTION_MARK || codePoint == Constants.CODE_INVERTED_EXCLAMATION_MARK || Character.getType(
            codePoint
     ...
     * [TextUtils.CAP_MODE_CHARACTERS], [TextUtils.CAP_MODE_WORDS], and
    ...
    fun getCapsMode(
        ...
        if (spacingAndPunctuations.mUsesAmericanTypography) {
            while (j > 0) {
                ...
                val c = cs[j - 1]
                if (c.code != Constants.CODE_DOUBLE_QUOTE && c.code != Constants.CODE_SINGLE_QUOTE && Character.getType(
                        c
        ...
        while (j > 0) {
            c = cs[--j]
            when (state) {
                START -> state = if (Character.isLetter(c)) {
                    WORD
                ...
                WORD -> state = if (Character.isLetter(c)) {
                    WORD
                ...
                PERIOD -> if (Character.isLetter(c)) {
                    state = LETTER
                } else {
                    return caps
                }

                LETTER -> state = if (Character.isLetter(c)) {
                    LETTER
                } else if (spacingAndPunctuations.isSentenceSeparator(c.code)) {
                    PERIOD
                } else {
                ...
                NUMBER -> state = if (Character.isLetter(c)) {
                    WORD
     ...
     * [TextUtils.CAP_MODE_CHARACTERS], [TextUtils.CAP_MODE_WORDS], and
     * [TextUtils.CAP_MODE_SENTENCES].
     * @return the text that describe the `capsMode`.
     */
