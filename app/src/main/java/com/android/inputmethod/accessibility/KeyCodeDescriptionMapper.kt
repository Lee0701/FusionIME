...
internal class KeyCodeDescriptionMapper private constructor() {
    ...
    private val mKeyCodeMap: SparseIntArray = SparseIntArray()
    ...
    fun getDescriptionForKey(
        ...
        if (code != Constants.CODE_UNSPECIFIED) {
            ...
            val isDefinedNonCtrl: Boolean = Character.isDefined(code)
                    && !Character.isISOControl(code)
            if (shouldObscure && isDefinedNonCtrl) {
                return context.getString(OBSCURED_KEY_RES_ID)
            }
            val description: String? = getDescriptionForCodePoint(context, code)
            if (description != null) {
                return description
            }
    ...
    fun getDescriptionForCodePoint(context: Context, codePoint: Int): String? {
        // If the key description should be obscured, now is the time to do it.
        val index: Int = mKeyCodeMap.indexOfKey(codePoint)
        if (index >= 0) {
            return context.getString(mKeyCodeMap.valueAt(index))
        }
        val accentedLetter: String? = getSpokenAccentedLetterDescription(context, codePoint)
        if (accentedLetter != null) {
            return accentedLetter
        }
        // Here, <code>code</code> may be a base (non-accented) letter.
        val unsupportedSymbol: String? = getSpokenSymbolDescription(context, codePoint)
        if (unsupportedSymbol != null) {
            return unsupportedSymbol
        }
        val emojiDescription: String? = getSpokenEmojiDescription(context, codePoint)
    ...
    private fun getSpokenAccentedLetterDescription(context: Context, code: Int): String? {
        ...
        val spokenText: String = context.getString(resId)
    ...
    private fun getSpokenSymbolDescription(context: Context, code: Int): String? {
        val resId: Int = getSpokenDescriptionId(context, code, SPOKEN_SYMBOL_RESOURCE_NAME_FORMAT)
        if (resId == 0) {
            return null
        }
        val spokenText: String = context.getString(resId)
    ...
    private fun getSpokenEmojiDescription(context: Context, code: Int): String? {
        val resId: Int = getSpokenDescriptionId(context, code, SPOKEN_EMOJI_RESOURCE_NAME_FORMAT)
        if (resId == 0) {
            return null
        }
        val spokenText: String = context.getString(resId)
    ...
    private fun getSpokenDescriptionId(
        context: Context, code: Int,
        resourceNameFormat: String
    ): Int {
        val resourceName: String = String.format(Locale.ROOT, resourceNameFormat, code)
        val resources: Resources = context.getResources()
        // Note that the resource package name may differ from the context package name.
        val resourcePackageName: String = resources.getResourcePackageName(
            R.string.spoken_description_unknown
        )
        val resId: Int = resources.getIdentifier(resourceName, "string", resourcePackageName)
        if (resId != 0) {
    ...
    companion object {
        private val TAG: String = KeyCodeDescriptionMapper::class.java.getSimpleName()
        private const val SPOKEN_LETTER_RESOURCE_NAME_FORMAT: String = "spoken_accented_letter_%04X"
        private const val SPOKEN_SYMBOL_RESOURCE_NAME_FORMAT: String = "spoken_symbol_%04X"
        private const val SPOKEN_EMOJI_RESOURCE_NAME_FORMAT: String = "spoken_emoji_%04X"
        private const val SPOKEN_EMOTICON_RESOURCE_NAME_PREFIX: String = "spoken_emoticon"
        private const val SPOKEN_EMOTICON_CODE_POINT_FORMAT: String = "_%02X"

        // The resource ID of the string spoken for obscured keys
        private val OBSCURED_KEY_RES_ID: Int = R.string.spoken_description_dot

        private val sInstance: KeyCodeDescriptionMapper = KeyCodeDescriptionMapper()

        fun getInstance(): KeyCodeDescriptionMapper {
            return sInstance
        }
         ...
         * @return a character sequence describing the action performed by pressing the key
         */
        private fun getDescriptionForSwitchAlphaSymbol(
            context: Context,
            keyboard: Keyboard
        ): String? {
         ...
         */
        private fun getDescriptionForShiftKey(
            context: Context,
            keyboard: Keyboard
        ): String {
            val keyboardId: KeyboardId = keyboard.mId
         ...
         */
        private fun getDescriptionForActionKey(
            context: Context, keyboard: Keyboard,
            key: Key
        ): String {
            val keyboardId: KeyboardId = keyboard.mId
            ...
            when (actionId) {
                EditorInfo.IME_ACTION_SEARCH -> resId = R.string.spoken_description_search
                EditorInfo.IME_ACTION_GO -> resId = R.string.label_go_key
                EditorInfo.IME_ACTION_SEND -> resId = R.string.label_send_key
                EditorInfo.IME_ACTION_NEXT -> resId = R.string.label_next_key
                EditorInfo.IME_ACTION_DONE -> resId = R.string.label_done_key
                EditorInfo.IME_ACTION_PREVIOUS -> resId = R.string.label_previous_key
                else -> resId = R.string.spoken_description_return
            }
            return context.getString(resId)
        }

        // TODO: Remove this method once TTS supports emoticon verbalization.
        private fun getSpokenEmoticonDescription(
            context: Context,
            outputText: String
        ): String? {
            ...
            val textLength: Int = outputText.length
            var index: Int = 0
            while (index < textLength) {
                val codePoint: Int = outputText.codePointAt(index)
            ...
            val resourceName: String = sb.toString()
            val resources: Resources = context.getResources()
            // Note that the resource package name may differ from the context package name.
            val resourcePackageName: String = resources.getResourcePackageName(
                R.string.spoken_description_unknown
