...
class DictionaryService : Service() {
    ...
    override fun onCreate() {
        ...
        mExecutor = ThreadPoolExecutor(
            1,  /* corePoolSize */1,  /* maximumPoolSize */
            WORKER_THREAD_TIMEOUT_SECONDS.toLong(),  /* keepAliveTime */
    ...
    override fun onStartCommand(
        ...
        val self: DictionaryService = this
        if (SHOW_DOWNLOAD_TOAST_INTENT_ACTION == intent.getAction()) {
            val localeString: String? = intent.getStringExtra(LOCALE_INTENT_ARGUMENT)
    ...
    companion object {
        private val TAG: String = DictionaryService::class.java.getSimpleName()

        /**
         * The package name, to use in the intent actions.
         */
        private const val PACKAGE_NAME: String = "com.android.inputmethod.latin"

        /**
         * The action of the date changing, used to schedule a periodic freshness check
         */
        private val DATE_CHANGED_INTENT_ACTION: String = Intent.ACTION_DATE_CHANGED

        /**
         * The action of displaying a toast to warn the user an automatic download is starting.
         */
        /* package */
        val SHOW_DOWNLOAD_TOAST_INTENT_ACTION: String =
         ...
         */
        /* package */
        const val LOCALE_INTENT_ARGUMENT: String = "locale"
         ...
         */
        private val UPDATE_FREQUENCY_MILLIS: Long = TimeUnit.DAYS.toMillis(4)
         ...
         */
        private val MAX_ALARM_DELAY_MILLIS: Int = TimeUnit.HOURS.toMillis(6).toInt()
         ...
         */
        private val VERY_LONG_TIME_MILLIS: Long = TimeUnit.DAYS.toMillis(14)
         ...
         */
        val NO_CANCEL_DOWNLOAD_PERIOD_MILLIS: Long = TimeUnit.SECONDS.toMillis(30)

        private const val WORKER_THREAD_TIMEOUT_SECONDS: Int = 15

        fun dispatchBroadcast(context: Context, intent: Intent) {
            val action: String? = intent.getAction()
            if (DATE_CHANGED_INTENT_ACTION == action) {
            ...
            } else if (DictionaryPackConstants.UPDATE_NOW_INTENT_ACTION == action) {
                // Intent to trigger an update now.
                UpdateHandler.tryUpdate(context)
            } else if (DictionaryPackConstants.INIT_AND_UPDATE_NOW_INTENT_ACTION == action) {
                // Initialize the client Db.
                val mClientId: String = context.getString(R.string.dictionary_pack_client_id)
         ...
         */
        private fun checkTimeAndMaybeSetupUpdateAlarm(context: Context) {
            ...
            val alarmTime: Long = now + Random().nextInt(MAX_ALARM_DELAY_MILLIS)
            val updateIntent: Intent = Intent(DictionaryPackConstants.UPDATE_NOW_INTENT_ACTION)
            // Set the package name to ensure the PendingIntent is only delivered to trusted components
            updateIntent.setPackage(context.getPackageName())
            var pendingIntentFlags: Int = PendingIntent.FLAG_CANCEL_CURRENT
         ...
         */
        fun updateNowIfNotUpdatedInAVeryLongTime(context: Context?) {
